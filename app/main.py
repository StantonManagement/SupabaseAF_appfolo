...
def trigger_sync(request: SyncRequest):
    ...
    if not request.dataset or not isinstance(request.dataset, str):
        logger.warning(f"API REQUEST FAILED - Invalid dataset parameter: {request.dataset}")
        raise HTTPException(
    ...
    if request.dataset not in DETAILS:
        logger.warning(f"API REQUEST FAILED - Unsupported dataset: '{request.dataset}'")
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        logger.info(f"API RESPONSE SENDING - Dataset: '{request.dataset}', Result: {result}")
        return SyncResponse(**result)

    except ValueError as e:
        logger.error(f"API REQUEST FAILED - ValueError for dataset '{request.dataset}': {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except RuntimeError as e:
        logger.error(f"API REQUEST FAILED - RuntimeError for dataset '{request.dataset}': {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
    except Exception as e:
        logger.error(f"API REQUEST FAILED - Unexpected error for dataset '{request.dataset}': {str(e)}")
